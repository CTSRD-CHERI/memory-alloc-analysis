...
data0[y2] //= 2**20        # aspace-allocator b to mb
data0[y3] //= 2**20        # allocd b to mb
data0[y4] //= 2**30        # swept b to gb

#print(data0)

matplotlib.rc('font', size=11)
fig, ax = plt.subplots()
plt.plot(time_s, data0[y1], time_s, data0[y2], time_s, data0[y3])
plt.title('{0} allocator address space usage over time'.format(data0_label))
plt.legend(['Aspace total', 'Aspace of allocator', 'Allocd by allocator'])
plt.xlabel('Time (s)')
